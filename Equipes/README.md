...
**Pour lancer le site en mode https:**

Générer un certificat de confiance avec mkcert (à faire une seule fois par poste de travail/VDI)

   Installer [mkcert](https://devportal.group.socgen/dev-tool/mkcert) installer du produit https://github.com/FiloSottile/mkcert

   Générer l'autorité de certification (CA) via la commande `mkcert -install`

   Générer un certificat : `mkcert -cert-file localhost-cert.pem -key-file localhost-key.pem serveurdev.sogecap.socgen localhost 127.0.0.1 ::1`

- Copier les fichiers localhost-cert.pem et localhost-key.pem dans un dossier ssl à créer dans le répertoire config de notre projet (dossier ignoré par git)

- Copier au même endroit le fichier rootCA.pem présent dans le dossier des données mkcert (chemin retrouvé avec la commande `mkcert --CAROOT`)
